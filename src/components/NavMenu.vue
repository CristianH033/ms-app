...
<template>
  <nav
    class="px-4 py-2 col-span-3 row-start-3 row-end-3 w-full h-16 mx-0 flex flex-row items-center justify-between sticky bottom-0 z-10 pb-safe border-t bg-background/90 text-foreground backdrop-blur md:col-span-1 md:row-start-2 md:row-end-4 md:col-start-1 md:col-end-1 md:h-fit md:flex-col md:items-stretch md:top-16 md:bottom-[inherit] md:px-4 md:py-4 md:border-t-0 md:min-h-[calc(100dvh-4rem)] md:border-r md:backdrop-blur-none md:bg-transparent"
  >
      ...
      >
        <RouterLink :to="{ name: item.pathName }" custom v-slot="{ isActive, navigate }">
          <Button
            ...
            role="link"
            variant="ghost"
            class="w-full p-1 flex flex-col justify-center overflow-hidden items-center hover:bg-primary/30 md:flex-row md:justify-start h-full md:h-12 md:p-2 md:px-4 md:gap-2"
            :class="{
          ...
          >
            ...
            >
              <span class="text-xs md:text-sm md:block">{{ item.name }}</span>
            </div>
          </Button>
        </RouterLink>
      </li>
